...
class CandidateController {
    constructor(candidateService) {
        ...
        this.getCandidateProfile = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await this.candidateService.getCandidateProfile(id);
                const { passwordHash, ...candidateWithoutPassword } = result.candidate;
                res.json({ candidate: candidateWithoutPassword, profile: result.profile });
            }
            catch (error) {
        ...
        this.updateProfile = async (req, res, next) => {
            try {
                ...
                const schema = zod_1.z.object({
                    ...
                    academicInfo: zod_1.z.any().optional(),
                    professionalInfo: zod_1.z.any().optional(),
        ...
        this.extractCV = async (req, res, next) => {
            try {
                ...
                const result = await this.candidateService.extractCVData(req.file.buffer, req.file.mimetype);
                res.json(result);
            }
            catch (error) {
                next(error);
