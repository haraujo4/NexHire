import { IAIProvider, AIInput, AIResult, CVExtractedData } from "../../core/interfaces/IAIProvider";
...
export class GeminiProvider implements IAIProvider {
    ...
    async analyzeCandidate(input: AIInput): Promise<AIResult> {
        const prompt = `
    ...
    async extractDataFromCV(text: string): Promise<CVExtractedData> {
        const prompt = `
    ...
    `;
        ...
        try {
            ...
            return JSON.parse(responseText) as CVExtractedData;
        } catch (error: any) {
    ...
    async summarizeProfile(profileData: any): Promise<{ general: string; experience: string; academic: string }> {
        const prompt = `
