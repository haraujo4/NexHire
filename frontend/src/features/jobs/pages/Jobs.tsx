    ...
    const fetchJobs = async () => {
        try {
            const [jobsData, appsData] = await Promise.all([
                jobsService.listCandidateJobs(),
                applicationsService.listCandidateApplications()
            ])

            const appliedJobIds = new Set(appsData.map((app: any) => app.jobId))
                        ...
                        <div key={job.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-gray-600 mt-1">{job.company?.name || 'NexHire'} • {job.location || 'Brasil'}</p>
                                </div>
