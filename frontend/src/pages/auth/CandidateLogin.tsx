...
export function CandidateLogin() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            const response = await api.post('/candidate/login', { email, password });
        ...
        <div className="min-h-screen bg-[var(--color-background)] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                ...
                <p className="mt-2 text-center text-sm text-gray-600">
                    Ainda não tem conta?{' '}
                    <Link to="/auth/candidate/register" className="font-medium text-[var(--color-primary)] hover:text-blue-500">
                        Cadastre-se grátis
                    </Link>
                </p>
            </div>
