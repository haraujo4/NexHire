        ...
        const fetchAll = async () => {
            try {
                ...
                const allApps = await Promise.all(
                    jobIds.map((id: string) => api.get(`/company/jobs/${id}/applications`))
                )

                const flattened = allApps.flatMap(res => res.data)
                setApplications(flattened.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()))
            } catch (error) {
